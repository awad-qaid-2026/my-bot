...
# القنوات (تأكدت من يوزر القناة الثانية jzbznznx)
...
def start(message):
    ...
    if is_subscribed(user_id):
        ...
        bot.send_message(message.chat.id, "⚔️ **أهلاً بك يا عواد في دمار المقنع!**\nاختر الخدمة المطلوبة الآن:", reply_markup=markup, parse_mode="Markdown")
    else:
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton("📢 انضم للقناة 1", url="https://t.me/v_o_lti"),
            types.InlineKeyboardButton("📢 انضم للقناة 2", url="https://t.me/jzbznznx"), # الرابط المصلح
        ...
        bot.send_message(message.chat.id, "⚠️ **عذراً! يجب الاشتراك في القنوات أولاً:**", reply_markup=markup, parse_mode="Markdown")
...
def handle_fetch(call):
    ...
    if nums:
        ...
        bot.answer_callback_query(call.id, "❌ حاول مرة أخرى لاحقاً", show_alert=True)
...
def verify_it(call):
    if is_subscribed(call.from_user.id):
        ...
        bot.answer_callback_query(call.id, "❌ لسه ما اشتركتش في كل القنوات!", show_alert=True)
